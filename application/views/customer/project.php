		...
		<div class="box box-info">
			<div class="box-header with-border">
				...
				<a href="javascript:void(0);"
				   onclick="loadPopup('<?= customer_url('addProject') ?>')"
...
<script>
	...
	window.onload = function () {
		Table = $('#reportTable').DataTable({
			...
			'bServerSide': true,
			'sAjaxSource': '<?= customer_url('getProjects') ?>',
			...
			buttons: [
				...
				},
				{
					...
					action: function (e, dt, node, config) {
						window.location = "<?= base_url('customer/archivesProject') ?>";
					}
				}
			]
		});
	...
	$('#reportTable tbody').on('click', 'tr td', function () {
		...
		switch (columnIndex) {
			case 4:
				break;
			default:
				window.location = "<?= base_url('customer/overviewProject/') ?>" + data.id;
				break;
		}
	});
