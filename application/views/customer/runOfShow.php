		...
		<div class="box box-info">
			<div class="box-header with-border">
				...
				<a href="javascript:void(0);"
				   onclick="loadPopup('<?= customer_url('addRunShow') ?>')"
...
<script>
	...
	window.onload = function () {
		Table = $('#reportTable').DataTable({
			...
			'bServerSide': true,
			'sAjaxSource': '<?= customer_url('getRunOfShow') ?>',
			...
			buttons: [
				...
				},
				{
					...
					action: function (e, dt, node, config) {
						window.location = "<?= base_url('customer/archives') ?>";
					}
				}
			]
		});
	...
	$('#reportTable tbody').on('click', 'tr td', function () {
		...
		switch (columnIndex) {
			case 5:
				break;
			default:
				window.location = "<?= base_url('customer/viewRunOfShowSchedule/') ?>" + data.id;
				break;
		}
	});
