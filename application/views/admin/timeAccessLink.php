							...
							<div class="box-header with-border"
								...
								<?php if ($data->archivesStatus == 0) { ?>
									<a href="javascript:void(0);" style="background-color: #2D2D2D; color: white"
									   onclick="loadPopup('<?= admin_url('addTimeAccessLink/') . $data->id ?>')"
														...
														<div class="dropdown">
															...
															<ul class="dropdown-menu">
																...
																<li><a href="javascript:void(0);"
																	   onclick="loadPopup('<?= base_url('admin/viewTimedAccessLink/' . $item->id) ?>')">View</a>
																</li>
																<li><a href="javascript:void(0);"
																	   onclick="loadPopup('<?= base_url('admin/editTimedAccessLink/' . $item->id) ?>')">Edit</a>
																</li>
																<li>
																	<a href="<?= base_url('admin/deleteTimedAccessLink/' . $item->id) ?>"
...
<script>
	...
	$(document).ready(function () {
		...
		$('#timedAccessLink tbody').on('change', '.status', function () {
			...
			$.ajax({
				url: '<?= admin_url("updateTimedAccessLinkStatus") ?>',
				type: 'POST',
				data: {
					id: id,
					status: status
	...
	$('#timedAccessLink tbody').on('click', 'tr td', function () {
		...
		switch (columnIndex) {
			...
			case 8:
				break;
			default:
				loadPopup('<?= admin_url('viewTimedAccessLink/') ?>' + data[0]);
				break;
		}
	});
</script>
