												...
												if ($detail['iOkr']):
													...
													<tr class="item-row" data-item-id="<?= $detail['iId'] ?>"
														...
														<td class="<?= $detail['iStatus'] == 'Completed' ? 'bg-green-active' : ($detail['iStatus'] == 'In-Progress' ? 'bg-light-blue' : ($detail['iStatus'] == 'Blocked' ?
															'bg-red' : ($detail['iStatus'] == 'Late' ? 'bg-yellow' : 'bg-gray'))) ?>"><?= $detail['iStatus'];
...
<script>
	...
	$(document).on('click', '.title-row td', function () {
		...
		if (allowedColumns.includes(columnIndex)) {
			...
			loadPopup('<?= admin_url('editProjectKpiTitle/') ?>' + titleId);
		}
	});
	...
	$(document).on('click', '.item-row td', function () {
		...
		if (allowedColumns.includes(columnIndex)) {
			...
			loadPopup('<?= admin_url('editProjectKpiItem/') ?>' + itemId);
		}
	});
	...
	$(document).on('change', '.titleTimelineView-checkbox', function () {
		...
		$.ajax({
			url: '<?= admin_url('changeTitleTimelineView') ?>',
			type: 'POST',
			data: {
				id: recordId,
				value: isChecked,
	...
	$(document).on('change', '.titleMilestoneMark-checkbox', function () {
		...
		console.log(isChecked);
		$.ajax({
			url: '<?= admin_url('changeTitleMilestoneMark') ?>',
			type: 'POST',
			data: {
				id: recordId,
				value: isChecked,
	...
	$(document).on('change', '.itemTimelineView-checkbox', function () {
		...
		console.log(isChecked);
		$.ajax({
			url: '<?= admin_url('changeItemTimelineView') ?>',
			type: 'POST',
			data: {
				id: recordId,
				value: isChecked,
	...
	$(document).on('change', '.itemMilestoneMark-checkbox', function () {
		...
		console.log(isChecked);
		$.ajax({
			url: '<?= admin_url('changeItemMilestoneMark') ?>',
			type: 'POST',
			data: {
				id: recordId,
				value: isChecked,
