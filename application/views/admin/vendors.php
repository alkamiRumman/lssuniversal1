...
<script>
	...
	window.onload = function () {
		Table = $('#item-list').DataTable({
			...
			"columnDefs": [
				{
					"render": function (data, type, row) {
						if (data) {
							return '<button class="btn btn-link" onclick="loadPopup(\'' + '<?= admin_url('viewVendorInvoiceW9Form/') ?>' + row['id'] + '\')">' + data + '</button>';
						} else {
							return '';
						}
					},
			...
			'bServerSide': true,
			'sAjaxSource': '<?= admin_url('getVendors') ?>',
	...
	$('#item-list tbody').on('click', 'tr td', function () {
		...
		switch (columnIndex) {
			...
			case 13:
				break;
			default:
				loadPopup('<?= admin_url('viewCustomer/') ?>' + data.id);
				break;
		}
	});
</script>
